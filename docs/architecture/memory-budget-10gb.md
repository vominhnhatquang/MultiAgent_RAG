...
|---------|-----------|---------|---------------------|
| **Ollama** | **2.2 GB** | nomic (0.3) + gemma2 (1.6) + overhead (0.3) | `mem_limit: 2500m` |
| PostgreSQL | 0.8 GB | shared_buffers=256MB, work_mem=4MB | `mem_limit: 800m` |
| Qdrant | 0.8 GB | HNSW cache + disk storage | `mem_limit: 800m` |
| Backend (FastAPI) | 0.5 GB | 2 Uvicorn workers | `mem_limit: 500m` |
| Frontend (Next.js) | 0.3 GB | SSR runtime | `mem_limit: 300m` |
| OS + Buffer | 1.1 GB | Kernel, filesystem cache, spikes | - |
| **Phase 1 Total** | **5.7 GB** | | |
...
| Celery Worker | +0.2 GB (new) | **0.2 GB** | 1 worker, concurrency=2 |
| **Phase 2 Total** | | **6.6 GB** | |
...
| Frontend (Next.js) | 0.3 GB | 3% |
| Celery Worker | 0.2 GB | 2% |
| OS + Buffer | 1.1 GB | 11% |
| **SUBTOTAL (Normal)** | **6.6 GB** | 66% |
| **Headroom for spikes** | **3.4 GB** | 34% |
...
| Frontend | 0.3 GB | |
| Celery | 0.2 GB | |
| OS + Buffer | 0.9 GB | Reduced during swap |
| **PEAK TOTAL** | **9.5 GB** | 0.5GB headroom |
...
| gemma2:2b | 1.6 GB | < 5s | 1+ | Default loaded |
...
```
NORMAL STATE (6.6GB total):
  Ollama: nomic(0.3) + gemma2(1.6) + reranker(0.4) = 2.3GB loaded

SWAP SEQUENCE (khi can llama3.1):
  1. Block new requests (queue them)
  2. Unload gemma2:2b        -> Free 1.6GB -> Ollama = 0.7GB
  3. Load llama3.1:8b        -> Alloc 4.7GB -> Ollama = 5.4GB
  4. Generate response        -> ~30s
  5. Unload llama3.1:8b      -> Free 4.7GB -> Ollama = 0.7GB
  6. Load gemma2:2b          -> Alloc 1.6GB -> Ollama = 2.3GB
  7. Unblock requests

  Peak during swap: 5.4GB Ollama + 3.6GB others = 9.0GB
  SAFE: 1.0GB headroom
```
...
ollama:
  ...
  deploy:
    resources:
      limits:
        memory: 6500m    # Hard limit for safety
      reservations:
        memory: 2500m    # Guaranteed minimum
  environment:
    - OLLAMA_NUM_PARALLEL=2       # Max concurrent requests
    - OLLAMA_MAX_LOADED_MODELS=3  # nomic + gemma2/llama + reranker
...
```
gemma2:2b concurrent performance:
...
|----------|-------------|--------|
| Idle (no sessions) | ~4.5 GB | OK |
| 1 active chat (gemma2) | ~6.6 GB | OK |
| 5 active chats (gemma2) | ~6.7 GB | OK |
| 1 swap to llama3.1 | ~9.0 GB | OK (1.0 headroom) |
| 5 chats + 1 swap | ~9.2 GB | WARNING |
| Document ingestion (100 chunks) | ~7.0 GB | OK |
| Ingestion + 3 chats | ~7.2 GB | OK |
| Ingestion + swap | ~9.5 GB | CRITICAL (0.5 headroom) |

---

## 7. OOM Prevention Rules

1. **Never load gemma2 + llama3.1 simultaneously** (1.6+4.7=6.3GB, exceeds Ollama budget with other models)
...
7. **Celery worker concurrency: 2** - limit parallel ingestion
...
```json
{
  ...
  "memory": {
    ...
    "available_gb": 3.2,
    "services": {
      "ollama": {"used_mb": 2400, "limit_mb": 6500, "models_loaded": ["nomic", "gemma2", "reranker"]},
